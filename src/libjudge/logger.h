...
#define log_debug(fmt, ...) do { \
	...
	if (isatty(fileno(_log_file))) { \
		fprintf(_log_file, STYLE_BRIGHT FORE_GREEN "[DEBUG %s:%d] " fmt "\n" STYLE_RESET, __FILE__, __LINE__, ##__VA_ARGS__); \
	} else { \
...
#define log_info(fmt, ...) do { \
	...
	if (_log_file == NULL) break; \
	if (isatty(fileno(_log_file))) { \
		fprintf(_log_file, STYLE_BRIGHT FORE_BLUE "[INFO %s:%d] " fmt "\n" STYLE_RESET, __FILE__, __LINE__, ##__VA_ARGS__); \
	} else { \
...
#define log_warn(fmt, ...) do { \
	...
	if (_log_file == NULL) break; \
	if (isatty(fileno(_log_file))) { \
		fprintf(_log_file, FORE_YELLOW "[WARN %s:%d] " fmt "\n" STYLE_RESET, __FILE__, __LINE__, ##__VA_ARGS__); \
	} else { \
