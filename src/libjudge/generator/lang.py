...
CONFIG_FILE = BASE_DIR + '/config/lang.yml'
HEADER_FILE = BASE_DIR + '/src/lang.h'
...
print >>h_file, '#define LANG_LENGTH	%d' % len(lang_def['languages'])
...
for name in lang_def['languages']:
...
for name in lang_def['languages']:
	...
	print >>h_file, 'const char *_compile_command%d[] = { %s, NULL }; ' % (idx, ', '.join(tokens))
	idx += 1

print >>h_file, 'const char **compile_command[] = {'

for i in range(idx):
	print >>h_file, '	_compile_command%d,' % i
...
print '%d language specification(s) generated' % len(lang_def['languages'])
